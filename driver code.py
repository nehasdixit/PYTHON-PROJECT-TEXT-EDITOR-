...
def main_window():
    ...
    text_area.pack(expand=tk.YES, fill=tk.BOTH,pady=40)#expand: adjusts to window size change
    text_area.insert(tk.END,"")#End: inserts text at the point where cursor was last
    #----------------------------------------------------------
    def pdf_inp(file):
        f=open(file,'rb')
        pdf_data=[]
    ...
    def italic():
        ...
        try:
            ...
            current_tags= text_area.tag_names("sel.first") #Returns a sequence of all tags associated with text_area starting at the index sel.first
    ...
    #-------------------------------------------
    editmenu = tk.Menu(menu, tearoff=0)  #tearoff: to remove dotted lines
...
splash_root.after(9000,main_window)
tk.mainloop()
